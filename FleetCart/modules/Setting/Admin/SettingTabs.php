...
class SettingTabs extends Tabs
{
    ...
    private function bkashPayment()
    {
        return tap(new Tab('bkash', trans('setting::settings.tabs.bkash')), function (Tab $tab) {
            $tab->weight(71);

            $tab->fields([
                'bkash_enabled',
                'translatable.bkash_label',
                'translatable.bkash_description',
                'bkash_test_mode',
                'bkash_app_key',
                'bkash_secret_key',
                'bkash_username',
            ...
            $tab->view('setting::admin.settings.tabs.bkash');
        });
    }

    private function nagadPayment()
    {
        return tap(new Tab('nagad', trans('setting::settings.tabs.nagad_payment')), function (Tab $tab) {
            $tab->weight(71);

            $tab->fields([
                'nagad_enabled',
                'translatable.nagad_label',
                'translatable.nagad_description',
                'nagad_test_mode',
                'nagad_merchant_id',
            ...
            $tab->view('setting::admin.settings.tabs.nagad');
        });
    }

    private function sslCommerz()
    {
        return tap(new Tab('sslcommerz', trans('setting::settings.tabs.sslcommerz')), function (Tab $tab) {
            $tab->weight(71);

            $tab->fields([
                'sslcommerz_enabled',
                'translatable.sslcommerz_label',
                'translatable.sslcommerz_description',
                'sslcommerz_test_mode',
                'sslcommerz_store_id',
                'sslcommerz_store_password',
                'sslcommerz_is_localhost'
            ]);

            $tab->view('setting::admin.settings.tabs.sslcommerz');
        });
    }
